...
(function (globals) {
    ...
    //@author lotas
    gh.pulls.prototype.createRequest = function (baseBranch, pullBranch, title, body, callback) {
        var url = "pulls/" + this.user + "/" + this.repo;

        $.post(apiRoot + url, {
            'login': authUsername,
            'token': authToken,
            'pull[base]' : baseBranch,
            'pull[head]' : pullBranch,
            'pull[title]' : title,
            'pull[body]' : body
            }, 
            function(data){
                callback.call(this, data.pull)
            }
        );
        return this;
    };
